<!-- Fuente: https://www.freqtrade.io/en/stable/recursive-analysis/ -->
...
In addition to the recursive formula check, this command also carries out a simple lookahead bias check on the indicator values only. For a full lookahead check, use [Lookahead-analysis](../lookahead-analysis/).

> **📌 Running recursive-analysis via freqUI**
>
> `recursive-analysis` can also be ran through freqUI when running freqtrade in [webserver mode](../utils/#webserver-mode).
...
`recursive-analysis` can also be triggered through the REST API when running freqtrade in [webserver mode](../utils/#webserver-mode).
As the analysis can run for a while, it is executed as a background task.
