<!-- Fuente: https://www.freqtrade.io/en/stable/backtesting/ -->
...
To learn how to get data for the pairs and exchange you're interested in, head over to the [Data Downloading](../data-download/) section of the documentation.

Backtesting is also available in [webserver mode](../freq-ui/#backtesting), which allows you to run backtests via the web interface.
...
For details on downloading, please refer to the [Data Downloading](../data-download/) section in the documentation.
...
> Please read the [pairlists documentation](../plugins/#pairlists) for more information.
>
> To achieve reproducible results, best generate a pairlist via the [`test-pairlist`](../utils/#test-pairlist) command and use that as static pairlist.

> **📌 Note**
>
> By default, Freqtrade will export backtesting results to `user_data/backtest_results`.
> The exported trades can be used for [further analysis](#further-backtest-result-analysis) or can be used by the [plotting sub-command](../plotting/#plot-price-and-indicators) (`freqtrade plot-dataframe`) in the scripts directory.
...
Backtesting supports [dynamic stake amount](../configuration/#dynamic-stake-amount) by configuring `stake_amount` as `"unlimited"`, which will split the starting balance into `max_open_trades` pieces.
...
Please also read about the [strategy startup period](../strategy-customization/#strategy-startup-period).
...
Reading the mean profit p-value
...
You can then load the trades to perform further analysis as shown in the [data analysis](../strategy_analysis_example/#load-backtest-results-to-pandas-dataframe) backtesting section.

Also, you can use freqtrade in [webserver mode](../freq-ui/#backtesting) to visualize the backtest results in a web interface.
...
In addition to the above assumptions, strategy authors should carefully read the [Common Mistakes](../strategy-customization/#common-mistakes-when-developing-strategies) section, to avoid using data in backtesting which is not available in real market conditions.
...
Extreme Difference Example

Using `--timeframe-detail` on an extreme example (all below pairs have the 10:00 candle with an entry signal) may lead to the following backtesting Trade sequence with 1 max\_open\_trades:
