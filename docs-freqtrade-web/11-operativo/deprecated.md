<!-- Fuente: https://www.freqtrade.io/en/stable/deprecated/ -->
...
Please refer to [pairlists](../plugins/#pairlists-and-pairlist-handlers) instead.
...
Please switch to the new [Parametrized Strategies](../hyperopt/) to benefit from the new hyperopt interface.
...
Please follow the [Strategy migration](../strategy_migration/) guide to migrate your strategy to the new format to start using the new functionalities.
...
version 2023.3 saw the removal of `populate_any_indicators` in favor of split methods for feature engineering and targets. Please read the [migration document](../strategy_migration/#freqai-strategy) for full details.
...
Please use the [`convert-data` subcommand](../data-download/#sub-command-convert-data) to convert your existing data to one of the supported formats before updating.
...
Please use configuration based [log setup](../advanced-setup/#advanced-logging) instead.
...
As the timeframe for both mark and funding\_fee candles has changed (usually from 8h to 1h) - already downloaded data will have to be adjusted or partially re-downloaded.
You can either re-download everything (`freqtrade download-data [...] --erase` - ![⚠](https://cdn.jsdelivr.net/gh/jdecked/twemoji@16.0.1/assets/svg/26a0.svg ":warning:") can take a long time) - or download the updated data selectively.
...
The result of the above will be that your funding\_rates and mark data will have the 1h timeframe.
you can verify this with `freqtrade list-data --exchange <yourexchange> --show`.

> **📌 Additional arguments**
>
> Additional arguments to the above commands may be necessary, like configuration files or explicit user\_data if they deviate from the default.
