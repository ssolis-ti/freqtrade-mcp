<!-- Fuente: https://www.freqtrade.io/en/stable/docker_quickstart/ -->
...
> You will find more information about Strategy development in the [Strategy documentation](../strategy-customization/).
...
UI Access on a remote server

If you're running on a VPS, you should consider using either a ssh tunnel, or setup a VPN (openVPN, wireguard) to connect to your bot.
This will ensure that freqUI is not directly exposed to the internet, which is not recommended for security reasons (freqUI does not support https out of the box).
Setup of these tools is not part of this tutorial, however many good tutorials can be found on the internet.
Please also read the [API configuration with docker](../rest-api/#configuration-with-docker) section to learn more about this configuration.
...
> If you intend to use freqUI, please also ensure to adjust the [configuration accordingly](../rest-api/#configuration-with-docker), otherwise the UI will not be available.
...
Head over to the [Data Downloading Documentation](../data-download/) for more details on downloading data.
...
Head over to the [Backtesting Documentation](../backtesting/) to learn more.
...
Commands `freqtrade plot-profit` and `freqtrade plot-dataframe` ([Documentation](../plotting/)) are available by changing the image to `*_plot` in your `docker-compose.yml` file.
