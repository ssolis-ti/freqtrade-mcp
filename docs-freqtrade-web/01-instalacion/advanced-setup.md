<!-- Fuente: https://www.freqtrade.io/en/stable/advanced-setup/ -->
...
For more information regarding usage of the sqlite databases, for example to manually enter or remove trades, please refer to the [SQL Cheatsheet](../sql_cheatsheet/).
...
using the sd\_notify (systemd notifications) protocol and will also tell systemd its current state (Running, Paused or Stopped)
when it changes.
...
> **📌 Note**
>
> The sd\_notify communication between the bot and the systemd service manager will not work if the bot runs in a Docker container.
...
>
> We recommend to extract the logging configuration from your main freqtrade configuration file, and provide it to your bot via [multiple configuration files](../configuration/#multiple-configuration-files) functionality. This will avoid unnecessary code duplication.
...
Deprecated - configure syslog via command line
...
Deprecated - configure journald via command line
...
The "fmt\_dict" attribute defines the keys for the json output - as well as the [python logging LogRecord attributes](https://docs.python.org/3/library/logging.html#logrecord-attributes).
