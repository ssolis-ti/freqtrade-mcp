<!-- Fuente: https://www.freqtrade.io/en/stable/strategy-101/ -->
...
[Freqtrade basics](../bot-basics/) page.
...
The number of slots is defined by the `max_open_trades` [configuration](../configuration/) option.
...
- there is already an open trade for a pair (Freqtrade cannot stack positions - however it can [adjust existing positions](../strategy-callbacks/#adjust-trade-position))
- if an entry and exit signal is present on the same candle, they are considered as [colliding](../strategy-customization/#colliding-signals), and no order will be raised
- the strategy actively rejects the trade order due to logic you specify by using one of the relevant [entry](../strategy-callbacks/#trade-entry-buy-order-confirmation) or [exit](../strategy-callbacks/#trade-exit-sell-order-confirmation) callbacks

Read through the [strategy customization](../strategy-customization/) documentation for more details.
...
- **backtesting**: using historical data that you [download from an exchange](../data-download/), backtesting is a quick way to assess performance of a strategy. However, it can be very easy to distort results so a strategy will look a lot more profitable than it really is. Check the [backtesting documentation](../backtesting/) for more information.
...
Dry runs are enabled by setting `dry_run` to true in your [configuration](../configuration/#using-dry-run-mode).

> **📌 Backtests can be very inaccurate**
>
> There are many reasons why backtest results may not match reality. Please check the [backtesting assumptions](../backtesting/#assumptions-made-by-backtesting) and [common strategy mistakes](../strategy-customization/#common-mistakes-when-developing-strategies) documentation.
...
Freqtrade includes two useful commands to check for basic flaws in strategies: [lookahead-analysis](../lookahead-analysis/) and [recursive-analysis](../recursive-analysis/).
...
- **[FreqUI](../freq-ui/)**: The easiest to get started with, FreqUI is a web interface to see and control current activity of your bot.
- **[Telegram](../telegram-usage/)**: On mobile devices, Telegram integration is available to get alerts about your bot activity and to control certain aspects.
...
- **[freqtrade-client](../rest-api/#consuming-the-api)**: A python implementation of the REST API, making it easy to make requests and consume bot responses from your python apps or the command line.
- **[REST API endpoints](../rest-api/#available-endpoints)**: The REST API allows programmers to develop their own tools to interact with a Freqtrade bot.
- **[Webhooks](../webhook-config/)**: Freqtrade can send information to other services, e.g. discord, by webhooks.
...
Check the [FAQ](../faq/#how-do-i-search-the-bot-logs-for-something) for examples.
...
Common questions and answers are available on our [FAQ](../faq/).

To continue, refer to the more in-depth [Freqtrade strategy customization documentation](../strategy-customization/).