<!-- Fuente: https://www.freqtrade.io/en/stable/trade-object/ -->
...
It will be passed to the strategy in many [strategy callbacks](../strategy-callbacks/). The object passed to the strategy cannot be modified directly. Indirect modifications may occur based on callback results.
...
| `close_rate` | float | Close rate - only set when is\_open = False. |
...
> During backtesting, it's limited to usage in [strategy callbacks](../strategy-callbacks/). Usage in `populate_*()` methods is not supported and will result in wrong results.
...
### total\_open\_trades\_stakes

Retrieve the total stake\_amount that's currently in trades.
...
| `safe_amount` | float | Amount - falls back to ft\_amount if None |
| `safe_price` | float | Price - falls back through average, price, stop\_price, ft\_price |
