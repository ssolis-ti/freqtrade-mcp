<!-- Fuente: https://www.freqtrade.io/en/stable/strategy-customization/ -->
...
- the [Freqtrade strategy 101](../strategy-101/), which provides a quick start to strategy development
- the [Freqtrade bot basics](../bot-basics/), which provides overall info on how the bot operates
...
Check the [configuration documentation](../configuration/) about how to set the bot to dry or live mode.
...
**For the following section we will use the [user\_data/strategies/sample\_strategy.py](https://github.com/freqtrade/freqtrade/blob/develop/freqtrade/templates/sample_strategy.py)
file as reference.**
...
[lookahead](../lookahead-analysis/) and [recursive](../recursive-analysis/) analysis.
...
>
> Look into the [user\_data/strategies/sample\_strategy.py](https://github.com/freqtrade/freqtrade/blob/develop/freqtrade/templates/sample_strategy.py).
...
You can use [recursive-analysis](../recursive-analysis/) to check and find the correct `startup_candle_count` to be used. When recursive analysis shows a variance of 0%, then you can be sure that you have enough startup candle data.
...
For the full documentation on stoploss features, look at the dedicated [stoploss page](../stoploss/).
...
Instead, please check the [Storing information](../strategy-advanced/#storing-information-persistent) section.
...
Informative\_pairs can also provide a 3rd tuple element defining the candle type explicitly.
...
When hyperopting, use of the hyperoptable parameter `.value` attribute is not supported. Please use the `.range` attribute. See [optimizing an indicator parameter](../hyperopt/#optimizing-an-indicator-parameter) for more information.
...
Plotting with current\_whitelist
...
It's also not supported for FreqUI visualization in [webserver mode](../utils/#webserver-mode), as the configuration for webserver mode doesn't require a pairlist to be set.

### *get\_pair\_dataframe(pair, timeframe)*
...
> Within [callbacks](../strategy-callbacks/), you'll get the full timerange up to the current (simulated) candle.

### *get\_analyzed\_dataframe(pair, timeframe)*

This method is used by freqtrade internally to determine the last signal.
It can also be used in specific callbacks to get the signal that caused the action (see [Advanced Strategy Documentation](../strategy-advanced/) for more details on available callbacks).
...
> **📌 Availabiity of delisting information**
>
...
> Within [callbacks](../strategy-callbacks/), you'll get the wallet state corresponding to the actual simulated wallet at that point in the simulation process.
...
For a full list of available methods, please consult the [Trade object](../trade-object/) documentation.
...
- don't use `.iloc[-1]` or any other absolute position in the dataframe within `populate_` functions, as this will be different between dry-run and backtesting. Absolute `iloc` indexing is safe to use in callbacks however - see [Strategy Callbacks](../strategy-callbacks/).
...
>
> You should always use the two helper commands [lookahead-analysis](../lookahead-analysis/) and [recursive-analysis](../recursive-analysis/), which can each help you figure out problems with your strategy in different ways.
...
Your next step is to learn [how to use backtesting](../backtesting/).